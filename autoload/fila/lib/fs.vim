...
function! fila#lib#fs#open(path) abort
  return s:File.open(a:path)
endfunction

function! fila#lib#fs#create_file(path) abort
...
function! fila#lib#fs#create_directory(path) abort
  if filereadable(a:path) || isdirectory(a:path)
    throw s:Revelator.error(printf(
          \ '"%s" already exist',
          \ a:path,
...
function! fila#lib#fs#copy(src, dst) abort
  if filereadable(a:dst) || isdirectory(a:dst)
    ...
    elseif r ==# 'r'
      ...
      return fila#lib#fs#copy(a:src, new_dst)
    endif
  endif
...
function! fila#lib#fs#move(src, dst) abort
  ...
  elseif filereadable(a:dst) || isdirectory(a:dst)
    ...
    elseif r ==# 'r'
      ...
      return fila#lib#fs#move(a:src, new_dst)
    endif
  endif
  call s:create_parent_path_if_requested(a:dst)
  if filereadable(a:src)
...
function! fila#lib#fs#trash(path) abort
...
function! fila#lib#fs#remove(path) abort
  if filereadable(a:path) && !filewritable(a:path)
    throw s:Revelator.error(printf(
          \ '"%s" is not writable',
          \ a:path,
